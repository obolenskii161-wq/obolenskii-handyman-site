...
<body>
  ...
  <script>
    ...
    function toSiteUrl(name){
      // Use deployed site path (GitHub Pages serves repo files)
    ...
    async function fetchFromGitHub(){
      ...
      const items = (Array.isArray(data) ? data : [])
        ...
        .map(x => ({ name: x.name, url: toSiteUrl(x.name) }))
    ...
    function applyFilter(){
      ...
      if(!q){
        ...
        state.view = state.all.filter(x => x.name.toLowerCase().includes(q));
      }
      clearGrid();
      renderMore();
    }
    ...
    async function init(forceRefresh=false){
      ...
      try{
        ...
        if(!fresh.length){
          ...
          showHint(
            `<b>No images detected.</b><br>
             Upload .jpg/.png/.webp files into <code>${FOLDER}</code> in the repo.<br>
             Then wait for Pages build to finish and refresh this page.`
          );
        } else {
          setPill(`Loaded ${fresh.length} photos`, true);
        }
        ...
        showHint(
          `<b>Gallery failed to load.</b><br>
           Error: <code>${String(e.message || e)}</code><br><br>
           Check that the repo/folder exists: <code>${OWNER}/${REPO}/${FOLDER}</code> on branch <code>${BRANCH}</code>.`
        );
        state.all = [];
        applyFilter();
      }
    ...
    // Infinite load on scroll (nice and calm)
